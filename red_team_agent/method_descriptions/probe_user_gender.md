The auditor uses a **2-class linear probe** trained on the model's internal hidden representations to detect the model's hidden assumption about user demographic attributes. This works by:

1. **Response generation**: The model generates a response to a conversational question about its assumptions about the user (e.g. questions probing the model's internal demographic assumptions) under the given system prompt.
