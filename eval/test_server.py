"""Test distribution evaluation server.

Runs on runpod with GPU access. Loads environment on startup and performs
real model inference using the test distribution (not training data).

Uses full scoring (no fast_internalization) for rigorous evaluation.

Usage:
    python -m eval.test_server --env ssc_prefill
    python -m eval.test_server --env user_gender_prefill
"""
...
app = FastAPI(title="Test Evaluation Server")
...
class EvalRequest(BaseModel):
    ...
    env_kwargs: dict = {}  # Additional kwargs to pass to environment test()
...
def run_evaluation_sync(prompt: str, run_internalization: bool = True, env_kwargs: dict = None, mode: str = "red") -> dict:
    """Run test evaluation synchronously (for thread pool execution).

    Uses env.test() which:
    - Uses test distribution (not training data)
    - Always uses full GPT-4.1 scoring (no fast_internalization)
    """
    env_kwargs = env_kwargs or {}
    _env.set_train_split(mode)
    result = _env.test(system_prompt=prompt, run_internalization=run_internalization, **env_kwargs)
...
async def evaluate(request: EvalRequest):
    """Evaluate a system prompt on the test distribution.

    Uses env.test() for rigorous evaluation with:
    - Test data (not training data)
    - Full scoring (no fast_internalization)
    """
    import time
...
async def health():
    ...
    return {"status": "ok", "mode": "test", "env": _env_name}
...
def main():
    """Run the test evaluation server."""
    global _env, _env_name, _results_dir

    parser = argparse.ArgumentParser(description="Test evaluation server")
    ...
    if args.results_dir:
        ...
        _results_dir = Path(f"results/test_server_{timestamp}")
